...
/// </summary>
public sealed class WebAppFileProvider : PhysicalFileProvider, IWebAppFileProvider
{
    /// <summary>
    ...
    /// </returns>
    private static bool IsUncPath(string path) => Uri.TryCreate(path, UriKind.Absolute, out Uri? uri) && uri.IsUnc;
    ...
    /// </param>
    public void DirectoryMove(string sourceDirName, string destDirName) => Directory.Move(sourceDirName, destDirName);
    ...
    public void FileCopy(string sourceFileName, string destFileName, bool overwrite = false) => File.Copy(sourceFileName, destFileName, overwrite);
    ...
    public string GetDirectoryNameOnly(string path) => new DirectoryInfo(path).Name;
    ...
    public string? GetParentDirectory(string directoryPath) => Directory.GetParent(directoryPath)?.FullName;
    ...
    public byte[] ReadAllBytes(string filePath) => File.Exists(filePath) ? File.ReadAllBytes(filePath) : Array.Empty<byte>();
